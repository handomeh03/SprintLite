...
export async function addissue(req, res) {
  ...
  const user = req.user;
  const { sprintId, type, title, description, assignee, priority, status } = req.body;

  if (!projectid || !type || !title || !priority || !status) {
  ...
  if (sprintId && !mongoose.Types.ObjectId.isValid(sprintId)) {
    return res.status(422).send({ error: "Invalid sprint id" }); 
  }
  if (assignee && !mongoose.Types.ObjectId.isValid(assignee)) {
    return res.status(422).send({ error: "Invalid assignee id" }); 
  }
...
export async function deleteIssue(req, res) {
  ...
  try {
    ...
    return res.status(200).send({ message: "Issue deleted successfully", Issue: issue });
  } catch (error) {
