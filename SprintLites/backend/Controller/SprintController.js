...
export async function createSprint(req, res) {
  ...
  if (!name) {
    return res.status(422).send({ error: "please enter name of sprint" });
  }
  if (!startDate) {
    return res.status(422).send({ error: "please enter startDate of sprint" });
  }
  if (!endDate) {
    return res.status(422).send({ error: "please enter endDate of sprint" });
  }
  const sprint = {
    project:projectid,
    name,
...
export async function changeStatus(req, res) {
  ...
  if (!status) {
    return res.status(422).send({ error: "Please enter the status" });
  }
